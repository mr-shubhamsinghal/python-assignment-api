...
from api_app.utilities import (getShiftATime, convertSecToTime,
                               getShiftBTime, getShiftCTime,
                               getFileDateTime)
...
def ProductionJSONFile(startDate, startTime, endDate, endTime):
    ...
    return filter_obj_with_date_time(objs, startDate, startTime,
                                     endDate, endTime)
...
def filter_obj_with_date_time(objects, startDate, startTime, endDate, endTime):
    ...
    for obj in objects:
        ...
        if ((date >= startDate and date <= endDate) and
...
def getAverageBeltValue(data_list):
    ...
    for index in range(len(result_list)):
        total_belt1 = result_list[index]['avg_belt1']
        avg_belt1 = int(total_belt1/result_list[index]['item_count'])
        result_list[index]['avg_belt1'] = avg_belt1
        total_belt2 = result_list[index]['avg_belt2']
        avg_belt2 = int(total_belt2/result_list[index]['item_count'])
        result_list[index]['avg_belt2'] = avg_belt2
